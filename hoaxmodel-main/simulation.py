...
class Simulation:
    ...
    def __init__(self, G, initial_state, state_transition,
            stop_condition=None, name='', state_labels=None, state_colors=None):
        '''
        ...
        self._state_colors = state_colors or {}

        # Initialize value_index with all possible states for consistent coloring
        if state_labels is not None:
            for i, label in enumerate(state_labels):
                self._value_index[label] = i
    ...
    def _categorical_color(self, value):
        # Use custom color if provided, else fallback to colormap
        if value in self._state_colors:
            return self._state_colors[value]
    ...
    def draw(self, step=-1, labels=None, **kwargs):
        ...
        d = dict(nx.degree(self.G))
        nx.draw(self.G, pos=self._pos, node_color=node_colors, edge_color="lightgray", nodelist=d.keys(), node_size=[v * 10 for v in d.values()], **kwargs)
        nx.draw_networkx_labels(self.G, pos=self._pos, font_size=5*min(d.values()), font_color="black")
    ...
    def plot(self, min_step=None, max_step=None, labels=None, **kwargs):
        ...
        for label in labels:
            ...
            color = self._state_colors[label] if label in self._state_colors else self._cmap(self._value_index[label])
            plt.plot(x_range, series, label=label, color=color, **kwargs)
